...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    a_vol=inference["anterior"]
    p_vol=inference["posterior"]
    total_tol=inference["total"]
    draw.text((10, 0), "HippoVolume.AI", (255, 255, 255), font=header_font)
    draw.multiline_text((10, 90),f"Patient ID: {header.PatientID}\n Anterior volume:{a_vol}\n Posterior volume:{p_vol}\n Total volume:{total_tol}\n Slice number:{slice_nums}\n Total slices: {orig_vol.shape[2]}\n",(255, 255, 255),font=main_font)
    ...
    position=[10,340,670]
    for i in range(3):    
        slice_o=orig_vol[:,:,slice_nums[i]]
        nd_img = np.flip((slice_o/np.max(slice_o))*0xff).T.astype(np.uint8)
        # This is how you create a PIL image from numpy array
        pil_i = Image.fromarray(nd_img, mode="L").convert("RGB").resize(tuple(i * 9 for i in slice_o.shape))
        # Paste the PIL image into our main report image object (pimg)
        
        slice_p=pred_vol[:,:,slice_nums[i]]
        nd_img = np.flip((slice_p/np.max(slice_p))*0xff).T.astype(np.uint8)
        gr_r=list(nd_img.ravel())
        gr_g = [0] * slice_p.shape[0] * slice_p.shape[1]
        gr_b = [0] * slice_p.shape[0] * slice_p.shape[1]
        alpha=np.zeros(slice_p.shape[0] * slice_p.shape[1])
        alpha[nd_img.ravel()!=0]=125
        alpha=list(alpha.astype(np.uint8))
        img_p = Image.new("RGBA", slice_p.shape)
        img_p.putdata(list(zip(gr_r, gr_g, gr_b,alpha)))
        img_p=img_p.resize(tuple(i * 9 for i in slice_o.shape))
        
        pil_i.paste(img_p, (0, 0), img_p)
        pimg.paste(pil_i, (position[i], 280))
    return pimg
...
def get_series_for_inference(path):
    ...
    for slice in dicoms:
        if slice.SeriesDescription=='HippoCrop':
            series_for_inference.append(slice)
...
if __name__ == "__main__":
    ...
    report_save_path = r"/home/workspace/report.dcm"
